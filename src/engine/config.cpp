...
static int lua_setConfig( lua_State *state) {
    std::string l_name;
    std::string l_category;
    std::string l_data;

    if( !lua_isstring( state, 1) || !lua_isstring( state, 2) || !lua_isstring( state, 3) ) {
        printf( "lua_setConfig call wrong argument\n");
        return 0;
    }

    l_name = lua_tostring( state, 1);
    l_category = lua_tostring( state, 2);
    l_data = lua_tostring( state, 3);

    lua_config->set( l_name, l_data, l_category);
    return 0;
}
...
static int lua_getConfig( lua_State *state) {
    ...
    l_data = lua_config->get( l_name, l_category);

    lua_pushstring( state, l_data.c_str());
    return 1;
}
...
config::config() {
    ...
    // only one config exist
    lua_config = this;

    // try to add lua lib
    lua_loader_add( "config", &lua_config_install);
}
